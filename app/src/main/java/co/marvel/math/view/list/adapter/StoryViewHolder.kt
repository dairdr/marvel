...
class StoryViewHolder(itemView: View, private val listener: ResultOptionListener) :
        RecyclerView.ViewHolder(itemView), ResultViewHolder {
    private var resultOption: StoryOption? = null
    ...
    init {
        ...
        rootView?.setOnClickListener {
            resultOption?.let { option ->
                listener.onClickStory(option.id)
            }
        }
    ...
    override fun bindViews(option: ResultOption) {
        this.resultOption = option as StoryOption
        nameView?.text = itemView.resources.getString(R.string.notSpecify)
        descriptionView?.text = option.name
